...
class ReaderResponseFrame(object):
    ...
    def __init__(self, resp_bytes, offset=0):
        ...
        #print("\n READER RESPONSE ",resp_bytes,"\n" , "\n LENGTH OF BYTES",resp_bytes[0])
        ...
        #print("READER ADDS ",self.reader_addr)
        self.resp_cmd = resp_bytes[offset+2]
        #print("\nRESP CMD ",self.resp_cmd)
        self.result_status = resp_bytes[offset+3]
        #print("\nRESULT ST ",self.result_status)
...
class ReaderInfoFrame(ReaderResponseFrame):
    ...
    def __init__(self, resp_bytes):
        ...
        if len(self.data) >= 8:
            ...
            #print("\nBAND TYPE :",self.max_frequency, "  ",self.min_frequency," dmax ",self.data[4]," dmin ",self.data[5])
            #print("\nUSER BAND ",((dmaxfre & 0b11000000 ) >> 4) + ((dminfre & 0b11000000 ) >> 6))
            if self.max_frequency == 62 and self.min_frequency==0 and self.type==0x09:
                self.frequency_band = ReaderFrequencyBand(0)
            else:
                self.frequency_band = ReaderFrequencyBand(((dmaxfre & 0b11000000 ) >> 4) + ((dminfre & 0b11000000 ) >> 6))
            #print("\nBAND TYPE :",self.frequency_band)
    ...
    def get_regional_frequency(self, fnum):
        ...
        elif self.frequency_band is ReaderFrequencyBand.User_B:
            return 902.6 + fnum * 0.4
        else:
            return fnum
